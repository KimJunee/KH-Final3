...
const charts = (() => {
  ...
  const lineChart = document.querySelectorAll('[data-line-chart]'),
  ...
  for (let i = 0; i < pieChart.length; i++) {
    ...
    let data = JSON.parse(pieChart[i].dataset.pieChart),
        options = (pieChart[i].dataset.options != undefined) ? JSON.parse(pieChart[i].dataset.options) : '',
        seriesColor = pieChart[i].dataset.seriesColor,
        userColors;
    
    pieChart[i].classList.add('ct-pie-chart-' + i);
    ...
    if (seriesColor != undefined) {
      ...
      for (let n = 0; n < userColors.colors.length; n++) {
        css = `
          .ct-pie-chart-${i} .ct-series:nth-child(${n+1}) .ct-slice-pie {
            fill: ${userColors.colors[n]} !important;
          }
          .ct-pie-chart-${i} .ct-series:nth-child(${n+1}) .ct-slice-donut {
            stroke: ${userColors.colors[n]} !important;
          }
        `;
        style.appendChild(document.createTextNode(css));
