...
//				JSONObject childObj = (JSONObject) rootObj.get("data");
				...
				for (int i = 0; i < UDetailArray.size(); i++) {
					...
					UrbtyOfctlLttotPblancDetail ud = new UrbtyOfctlLttotPblancDetail(house_manage_no, pblanc_no,
							house_nm, house_secd, house_secd_nm, house_dtl_secd, house_dtl_secd_nm, search_house_secd,
							hssply_zip, hssply_adres, tot_suply_hshldco, rcrit_pblanc_de, subscrpt_rcept_bgnde,
							subscrpt_rcept_endde, przwner_presnatn_de, cntrct_cncls_bgnde, cntrct_cncls_endde,
							hmpg_adres, bsns_mby_nm, mdhs_telno, mvn_prearnge_ym);
					list.add(ud);
...
//				System.out.println(UDetailArray.toString());
				...
				for (int i = 0; i < UrbtyMdlArray.size(); i++) {
					...
					UrbtyMdl um = new UrbtyMdl(pblanc_no, house_manage_no, model_no, gp, tp, excluse_ar, suply_hshldco,
