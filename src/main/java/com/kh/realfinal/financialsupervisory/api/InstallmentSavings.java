...
public class InstallmentSavings {
	...
	public static void main(String[] args) {
		...
		InstallmentSavings.callInstallmentSavingsByXML();
		System.out.println("-----------------------------------------");
	}

	public static List<Map<String,Object>>callInstallmentSavingsByXML() {
						...
						if (node.getNodeType() == Node.ELEMENT_NODE) {
							...
							for (int k = 0; k < nOption.getLength(); k++) {
								...
								if (oNode.getNodeType() == Node.ELEMENT_NODE) {
									...
									option.put("rsrv_type", oElement.getElementsByTagName("rsrv_type").item(0).getTextContent());
									option.put("rsrv_type_nm", oElement.getElementsByTagName("rsrv_type_nm").item(0).getTextContent());
